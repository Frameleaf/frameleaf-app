...
Future<void> migrateDatabaseIfNeeded(Drift drift, NativeSyncApi nativeSyncApi, PermissionApi permissionApi) async {
  ...
  // Version 27 is already released in the fork for backup settings.
  if (version < 28) {
    await _migrateTo28(drift, nativeSyncApi, permissionApi);
  }
...
Future<void> _migrateTo28(Drift drift, NativeSyncApi nativeSyncApi, PermissionApi permissionApi) async {
  ...
  try {
    ...
    Logger('Migration').warning("Error migrating to version 28", error, stackTrace);
  }
}
