...
void main() {
  ...
  test('heals dates from MediaStore', () async {
    ...
    expect(Store.tryGet(StoreKey.version), 28);
  });

  test('repairs dates for existing version-27 fork users without overwriting backup preferences', () async {
    final takenAt = DateTime.utc(2019, 3, 15);
    await Store.put(StoreKey.version, 27);
    await Store.put(StoreKey.backupRequireCharging, true);
    await Store.put(StoreKey.backupTriggerDelay, 60);
    await Store.put(StoreKey.enableBackup, false);
    await ctx.newLocalAsset(id: 'local', createdAt: DateTime.utc(2026));
    when(() => nativeSyncApi.getAssetsForAlbum('album')).thenAnswer((_) async => [_asset('local', takenAt)]);
    ...
    expect(Store.tryGet(StoreKey.version), 28);
    expect(Store.tryGet(StoreKey.backupRequireCharging), true);
    expect(Store.tryGet(StoreKey.backupTriggerDelay), 60);
    expect(Store.tryGet(StoreKey.enableBackup), false);

    await migrateDatabaseIfNeeded(ctx.db, nativeSyncApi, permissionApi);
    verify(() => nativeSyncApi.getAlbums()).called(1);
  });

  test('retains the fork backup migration when upgrading from version 26', () async {
    await Store.put(StoreKey.version, 26);
    await Store.put(StoreKey.legacyForkBackupRequireCharging, true);
    await Store.put(StoreKey.legacyForkBackupTriggerDelay, 30);
    await Store.put(StoreKey.legacyForkSyncAlbums, true);
    await Store.put(StoreKey.legacyForkEnableBackup, true);
    await Store.put(StoreKey.legacyForkUseWifiForUploadVideos, true);
    await Store.put(StoreKey.legacyForkUseWifiForUploadPhotos, false);
    // An explicitly saved fork value must win over an old value.
    await Store.put(StoreKey.enableBackup, false);

    await migrateDatabaseIfNeeded(ctx.db, nativeSyncApi, permissionApi);

    expect(Store.tryGet(StoreKey.backupRequireCharging), true);
    expect(Store.tryGet(StoreKey.backupTriggerDelay), 30);
    expect(Store.tryGet(StoreKey.syncAlbums), true);
    expect(Store.tryGet(StoreKey.enableBackup), false);
    expect(Store.tryGet(StoreKey.useWifiForUploadVideos), true);
    expect(Store.tryGet(StoreKey.useWifiForUploadPhotos), false);
    expect(Store.tryGet(StoreKey.legacyForkBackupRequireCharging), isNull);
    expect(Store.tryGet(StoreKey.version), 28);
  });
