...
describe('importConfig (FL-66 unsupported imported leaves)', () => {
  ...
  it('keeps saved secrets when the file leaves them empty, and ignores server bookkeeping and revisions', () => {
    const result = importConfig(configFixture(), {
      revision: 'old',
      oauth: { clientSecret: '' },
    ...
    expect(result.draft.notifications.smtp.transport.password).toBe('from-file');
