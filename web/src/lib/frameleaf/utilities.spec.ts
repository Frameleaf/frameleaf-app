...
const sdk = vi.hoisted(() => ({ getDuplicateReview: vi.fn(), getDuplicateDecisions: vi.fn(), getLivePhotoCandidates: vi.fn(), listICloudConnections: vi.fn(), searchLargeAssets: vi.fn() }));
...
describe('Command Center utilities', () => {
  it('preserves the prototype groups and limits health tools to admins', () => {
    ...
    expect(utilityToolsFor(false).map((tool) => tool.id)).toEqual(['duplicates', 'large-files', 'geolocation', 'live-photos', 'icloud', 'workflows', 'downloads', 'obtainium']);
  ...
  it.each(['missing-media', 'corrupt-media'] as const)('rejects non-admin %s before requesting findings', async (tool) => {
    await expect(loadUtility(tool, new URL('https://example.test/user-settings'), false)).rejects.toMatchObject({ status: 403 });
  ...
  it('keeps bookmarked photo ids and review filters in the settings destination', async () => {
    await expect(redirectUtility(new URL('https://example.test/utilities/duplicates/photos/a?index=2&status=found'), 'duplicates', 'a')).rejects.toMatchObject({ status: 307, location: '/user-settings?area=utilities&section=duplicates&status=found&index=2&assetId=a' });
  });
});
