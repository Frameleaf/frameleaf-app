...
/**
 * Values the server keeps on its own: the re-queue reminder bookkeeping and the "a key is stored"
 * indicators. A save never changes them, so they are never a change, a conflict or journaled.
 */
...
/** Credentials stored in the configuration. Shown only as "changed", never journaled or exported. */
...
export const redactConfigForExport = <T extends object>(config: T): T => {
  ...
  for (const [path, value] of configLeaves(next)) {
    if (isServerManagedConfigPath(path)) {
      ...
      set(next, path, value.map((item) => (typeof item === 'string' ? stripUrlCredentials(item) : item)));
    }
  }
  return next as T;
};
 ...
 * of the same kind, are applied; everything else is reported and ignored. Empty secrets keep the
 * saved secret. Server-kept values are ignored. Nothing is sent to the server: the administrator
...
export const importConfig = <T extends object>(draft: T, imported: unknown): ConfigImportResult<T> => {
  ...
  return { draft: next, applied, unsupported, keptSecrets };
};
