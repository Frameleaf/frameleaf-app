 ...
 */
export type PasswordRuleId = 'minLength' | 'lower' | 'upper' | 'number' | 'symbol';

export const PASSWORD_RULES: readonly { id: PasswordRuleId; labelKey: string; test: (value: string) => boolean }[] =
  Object.freeze([
    { id: 'minLength', labelKey: 'frameleaf_password_rule_length', test: (value) => value.length >= 8 },
...
export const passwordStrength = (password: string): PasswordStrength => {
  ...
  const longEnough = checks.minLength;
  let score = 0;
  if (value.length > 0) {
    score = longEnough ? clamp(passed - 1, 1, 4) : 1;
    if (longEnough && value.length >= 14) {
  ...
  return {
    ...
    checks,
    acceptable: longEnough && passed >= 3,
  };
};
