...
function setup(isTrashed = true) {
  ...
  const interaction = new AssetMultiSelectManager();
  const view = renderWithTooltips(GalleryViewer, {
    assets,
    assetInteraction: interaction,
    viewport: { width: 1000, height: 800 },
  });
  return { assets, view, interaction };
}

it.each([true, false])(
  'advances after two real gallery deletions with deferred route publication (permanent=%s)',
  async (force) => {
    const { assets, view, interaction } = setup(force);
    const [a, b, c] = assets;
...
it('uses the previous neighbor for the last asset and ignores stale confirmation cursor', async () => {
  const { assets, view, interaction } = setup();
  const [a, b, c] = assets;
...
it('uses the current cursor after backward and forward navigation', async () => {
  const { assets, view, interaction } = setup(false);
  const [a, b, c] = assets;
