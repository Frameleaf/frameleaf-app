...
describe(StackService.name, () => {
  ...
  describe('create', () => {
    ...
    it('follows up on photos a Locked stack member locked with it (FL-53)', async () => {
      ...
      // face thumbnails and profile pictures released by the photos the stack moved into Locked
      expect(mocks.person.getMissingThumbnailsForAssets).toHaveBeenCalledWith([primaryAsset.id, asset.id]);
