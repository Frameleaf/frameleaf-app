...
const sessionStub = (worker: RenderWorker, token: string, overrides: Partial<RenderWorkerSession> = {}): RenderWorkerSession =>
  ({
    id: `session-${token}`,
    workerId: worker.id,
    token: hashed(token),
...
describe(RenderWorkerService.name, () => {
  ...
  const workerB = workerStub({ id: '0195e2a0-0000-7000-8000-00000000bbb2', name: 'Attic GPU', enrolmentSecret: hashed('secret-b') });
  ...
  describe('create', () => {
    it('stores only the digest of the enrolment secret and returns the secret once', async () => {
      ...
      expect(workers.createWorker).toHaveBeenCalledWith(
        expect.objectContaining({ enrolmentSecret: hashed(response.enrolmentSecret), createdBy: authStub.admin.user.id }),
      );
  ...
  describe('claim', () => {
    ...
    it('hands out the claim with grants only for inputs the owner can still read', async () => {
      ...
      expect(claim!.inputs[0]).toEqual(
        expect.objectContaining({ inputId: 'source', assetId: queued.assetId, role: 'source' }),
      );
      ...
      expect(workers.claimQueued).toHaveBeenCalledWith(expect.objectContaining({ id: queued.id, workerId: workerA.id }));
    });
  ...
  describe('guarded writes', () => {
    ...
    beforeEach(() => {
      ...
      vi.mocked(workers.getClaimed).mockImplementation((id, workerId, claimToken) =>
        Promise.resolve(
          id === claimedByA.id && workerId === workerA.id && claimToken === 'claim-1' ? (claimedByA as never) : undefined,
        ),
      );
    });
    ...
    it('refuses the claiming worker presenting a stale or guessed token', async () => {
      await expect(sut.progress(SESSION_A, claimedByA.id, { claimToken: 'claim-0', status: 'rendering', processedUnits: 1, totalUnits: 2 } as never)).rejects.toBeInstanceOf(NotFoundException);
    ...
    it('stops an operation over the account’s wall clock on progress', async () => {
      vi.mocked(workers.getLimit).mockImplementation((subject) =>
        Promise.resolve(
          subject === OWNER_A ? ({ subject, maxConcurrentOperations: 2, maxWallClockMs: '1000', maxOutputBytes: null } as never) : undefined,
        ),
      );
    ...
    it('passes checkpoints through under the claim token only', async () => {
      ...
      expect(operations.completeCheckpoint).toHaveBeenCalledWith(
        claimedByA.id,
        'claim-1',
        expect.objectContaining({ sequence: 0, chunkKey: 'k', outputChecksum: Buffer.from('abcd', 'hex'), sizeInBytes: 42 }),
      );
    });

    it('only acknowledges a cancel on a job that is cancelling and held by this worker', async () => {
      expect(await sut.acknowledgeCancel(SESSION_A, claimedByA.id, { claimToken: 'claim-1', released: true } as never)).toEqual({
        accepted: false,
        refusal: null,
      });
      ...
      vi.mocked(workers.getClaimed).mockResolvedValue(
        operationStub({ ...claimedByA, status: MediaOperationStatus.Cancelling, cancelRequestedAt: new Date() }) as never,
      );
      expect(await sut.acknowledgeCancel(SESSION_A, claimedByA.id, { claimToken: 'claim-1', released: true } as never)).toEqual({
        accepted: true,
        refusal: null,
  ...
  describe('readInput', () => {
    ...
    const grantFor = (operation: MediaOperation, session: RenderWorkerSession, claimToken = 'claim-1') =>
      signInputGrant(
        { operationId: operation.id, inputId: 'source', assetId: operation.assetId!, expiresAt: Date.now() + 60_000 },
    ...
    it('refuses a grant for one operation replayed against another the same worker holds', async () => {
      ...
      vi.mocked(workers.getClaimedByWorker).mockImplementation((id) =>
        Promise.resolve(id === otherOp.id ? (otherOp as never) : id === claimedByA.id ? (claimedByA as never) : undefined),
      );
    ...
    it('refuses an operation this worker never claimed', async () => {
      const foreign = operationStub({ id: '0195e2a0-0000-7000-8000-000000000077', claimedBy: workerB.id, claimToken: 'x' });
  ...
  describe('limits', () => {
    ...
    it('keys a per-account ceiling by the user id', async () => {
      ...
      expect(workers.upsertLimit).toHaveBeenCalledWith(authStub.user1.user.id, authStub.user1.user.id, expect.anything());
    });
  });
});
