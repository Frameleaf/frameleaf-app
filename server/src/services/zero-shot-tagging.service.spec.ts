...
describe(ZeroShotTaggingService.name, () => {
  ...
  it('memoizes the vocabulary: one ML round-trip per model across calls', async () => {
    ...
    expect(mocks.machineLearning.encodeText).toHaveBeenCalledWith(expect.objectContaining({ destinationId: expect.any(String), workload: expect.any(String) }), expect.any(String), { modelName: 'memo-model' });
  });
