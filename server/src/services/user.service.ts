...
export class UserService extends BaseService {
  ...
  async handleUserDeleteCheck(): Promise<JobStatus> {
    ...
    } else if (swept.preferenceHistory + swept.recipientGroups + swept.peopleAndPets > 0) {
      this.logger.log(
        `Swept fork rows of removed accounts: ${swept.preferenceHistory} preference history entries, ${swept.recipientGroups} recipient groups, ${swept.peopleAndPets} face correction, merge answer and pet recognition rows`,
      );
    }
    return JobStatus.Success;
  }
