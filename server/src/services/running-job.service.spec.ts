...
describe(RunningJobService.name, () => {
  ...
  beforeEach(() => {
    mocks = getMocks();
    operations = { list: vi.fn().mockResolvedValue({ items: [], total: 0 }) } as unknown as MediaOperationRepository;
    ...
    sut = new RunningJobService(mocks.logger as never, mocks.job as never, operations, mocks.memory as never);
  });
