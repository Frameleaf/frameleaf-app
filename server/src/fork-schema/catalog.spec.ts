...
describe('catalog manifests', () => {
  ...
  it('derives deterministic complete lock sets for both installation classes', () => {
    ...
    // 126 public (v3.1.0, the post-certified residue cluster_group, cluster_group_request, person_group,
    // person_group_audit, workflow_log, and the legacy fork tables of the 2100000000NNN migrations, including
    // the three Studio export tables of 2100000000580) + 33 fork tables, including seven iCloud tables and
    // asset_develop_revision
    expect(getCatalogTableLocks(fork)).toHaveLength(159);
