...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await sql`ALTER TABLE "asset_file" ADD CONSTRAINT "asset_file_assetId_type_isEdited_uq" UNIQUE ("assetId", "type", "isEdited");`.execute(
    db,
  );
}
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  await sql`ALTER TABLE "asset_file" ADD CONSTRAINT "asset_file_assetId_type_uq" UNIQUE ("assetId", "type");`.execute(
    db,
  );
