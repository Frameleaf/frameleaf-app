...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await sql`ALTER TABLE "physical_file" RENAME CONSTRAINT "physical_file_path_unique" TO "physical_file_path_uq";`.execute(
    db,
  );
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  await sql`ALTER TABLE "physical_file" RENAME CONSTRAINT "physical_file_path_uq" TO "physical_file_path_unique";`.execute(
    db,
  );
