...
describe('exact v3.1.0 public schema cutover evidence', () => {
  ...
  it('acquires the exact sorted original-official manifest lock set before verification', async () => {
    ...
    // 66 v3.1.0 public + the 45 fork tables: the video version (FL-39), archive operation (FL-32),
    // merge verdict (FL-57), album position (FL-52), recipient group (FL-55), preference
    // history (FL-71), memory curation and show-less (FL-62), render worker session capability
    // (FL-95) and Studio workspace layout (FL-91) tables included
    expect(observedLocks).toHaveLength(111);
