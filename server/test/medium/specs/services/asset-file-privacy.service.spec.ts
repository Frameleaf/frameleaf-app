...
it('preserves owner access when hiding is disabled and denies legacy Locked files without elevation', async () => {
...
it('applies tag/person suppression to derivative access independently of sensitive classification', async () => {
  ...
  for (const rule of [
    { tagIds: [tag.id], personIds: [] },
    { tagIds: [], personIds: [person.personGroupId] },
  ]) {
    const auth = {
      ...locked,
