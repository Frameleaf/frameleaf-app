...
Here is a quick, no-choices path to install Frameleaf and take it for a test drive.
...
Try uploading a picture from your browser: select **Upload** and choose a file.
...
To see Frameleaf processing your photos, select **Settings** in the sidebar to open the Command Center, then open **Compute & jobs**.

---

## Review the database backup and restore process

Frameleaf has built-in database backups. You can refer to the
...
You may decide you'd like to add the _rest_ of your photos from Google Photos, even those not on your mobile device, via Google Takeout. Use [Import Google Photos](/features/google-photos-import) for this.
