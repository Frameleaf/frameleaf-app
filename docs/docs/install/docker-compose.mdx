...
Docker Compose is the recommended method to run Frameleaf. Use the files attached to a published [Frameleaf release](https://github.com/Frameleaf/frameleaf-app/releases); they select the Frameleaf server and machine-learning images.

For an existing installation, retain its Compose project name, `.env`, storage paths and volume names. The displayed container names change to `frameleaf_*`, while service names, database settings and `IMMICH_*` variables remain compatible. Review the [container migration notes](https://github.com/Frameleaf/frameleaf-app/blob/fork/main/docker/README.md) before upgrading; do not create a second stack against the same database directory.
