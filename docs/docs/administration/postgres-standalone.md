...
While not officially recommended, it is possible to run Frameleaf using a pre-existing Postgres server. To use this setup, you should have a baseline level of familiarity with Postgres and the Linux command line. If you do not have these, we recommend using the default setup with a dedicated Postgres container.

By default, Frameleaf expects superuser permission on the Postgres database and requires certain extensions to be installed. This guide outlines the steps required to prepare a pre-existing Postgres server to be used by Frameleaf.
...
Frameleaf is known to work with Postgres versions `>= 14, < 20`.

VectorChord is known to work with pgvector versions `>= 0.7, < 0.9`.

The Frameleaf server will check the VectorChord version on startup to ensure compatibility, and refuse to start if a compatible version is not found.
...
:::caution
This method is recommended for **advanced users only** and often requires manual intervention when updating Frameleaf.
:::
...
When installing a new version of VectorChord, you will need to manually update the extension and reindex by connecting to the Frameleaf database and running:
...
6. If Frameleaf does not have superuser permissions, run the SQL command `CREATE EXTENSION vchord CASCADE;` using psql or your choice of database client
7. Start Frameleaf and wait for the logs `Reindexed face_index` and `Reindexed clip_index` to be output
8. If Frameleaf does not have superuser permissions, run the SQL command `DROP EXTENSION vectors;`
...
5. Start Frameleaf and let it create new indices using VectorChord
...
3. If Frameleaf does not have superuser permissions, run the SQL command `CREATE EXTENSION vchord CASCADE;`
4. Remove the `DB_VECTOR_EXTENSION=pgvector` environmental variable as it will make Frameleaf still use pgvector if set
5. Start Frameleaf and let it create new indices using VectorChord
