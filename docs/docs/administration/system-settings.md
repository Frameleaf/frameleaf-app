...
- **Change history** lists every saved settings change, newest first, with the administrator who saved it, the time, and each setting before and after. It is kept on the server, so every administrator sees the same history (the latest 50 saves). Credentials appear only as replaced or cleared, and credentials inside addresses are removed.
