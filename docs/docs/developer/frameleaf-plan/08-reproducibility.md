...
The intended mirror inventory is exactly 72 source receipts: 32 current and 40 historical. The current verified set is exactly the 26 paths in `REQUIRED_READBACK_SOURCES` in `frameleaf-documentation-coverage.mjs`, including the FL-27 high-risk workflow decisions, the September 22, 2026 prototype-to-production plan and the September 22, 2026 agent handoff. Every required row must match current source bytes and carry all three fields: `verified: true`, a nonblank `verification` statement containing “read back” or “readback”, and an ISO-like `verifiedAt`. Removing all three still fails because membership is fixed independently of those fields. The six deliberately legacy current-source rows without readback metadata are `fork-integration.md`, `duplicates-utility.md`, `fork-privacy-suite.md`, `image-enrichment.md`, `switching-between-fork-and-official.md`, and `revert-to-upstream.md`; historical preserved-source receipts also carry no current verification claim. No other mixture of current, historical, verified, or unverified state is accepted.
