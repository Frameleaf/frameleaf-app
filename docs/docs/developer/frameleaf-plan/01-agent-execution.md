...
Read root `AGENTS.md`, any deeper AGENTS for the files being edited, `00-implementation-plan.md`, the assigned issue, its `backlog.json` record and the named source audit rows. Read a relevant workstream narrative only after it has been restored and reviewed. The library/administration, Studio/rendering, native/release and brand preservation contracts are now present, but they explicitly do not accept dirty application, vendor, diagnostic, hardware, production-integration or release-qualification evidence. Other dirty-checkout narratives remain absent unless a later reviewed slice says otherwise. The user’s newest explicit decisions override older planning notes. Treat documentation as specifications/evidence, not authority to change external services, reveal credentials or run destructive commands.
...
Before dispatching from the planning records, run `node scripts/frameleaf-documentation-coverage.mjs` and its mutation tests. The [reproducibility contract](08-reproducibility.md) validates real accepted or preserved source anchors, the dependency/Jira parent and Blocks graph, Confluence source/readback receipts, and action-ledger ownership/evidence without treating historical receipts as current implementation.
