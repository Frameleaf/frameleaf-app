...
The initial FL-25 baseline slice deliberately restored only the reviewed execution core. Later bounded slices restored the library/administration and Studio/rendering preservation contracts without importing or accepting the dirty application implementation. This slice restores the native/release narrative and ownership map in `04-native-and-release.md` and `native-issue-map.json`; they inventory committed and hashed dirty-only evidence without claiming implementation or qualification. Delivery sequencing, brand integration and feature-ownership records remain independently reviewed or pending according to their current source state. Their absence must not be interpreted as reduced product scope or permission to start a dependency early.
...
The user-supplied seven-SVG kit in `design/frameleaf/brand-kit` is the authoritative artwork. Preserve originals and record platform derivatives; earlier generated PNGs are references, not release masters. Follow the committed [design handoff](https://github.com/Frameleaf/frameleaf-app/blob/4bedc0c4d384f365ee5c29739ce201cc114fdd85/design/frameleaf/README.md), manifest and file hashes for actual variants, theme contrast and platform export requirements. The additional dirty-checkout brand narrative is not restored by this slice.
