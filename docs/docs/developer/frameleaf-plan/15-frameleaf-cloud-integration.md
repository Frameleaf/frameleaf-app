...
- **As built (FL-161, CLD-101)**, the API side of the via contract and the remote-access prerequisites, off the network until the edge worker (CLD-102) exists:
  - `middleware/frameleaf-via.middleware.ts`, registered in `app.common.ts` right after `cookieParser()`, reads the arrival (`frameleafVia()`, timing-safe against `FRAMELEAF_EDGE_SECRET`), sets `request.frameleafVia` (`lan|wan|relay`, else null) and `request.frameleafForwarded` (the nearest `X-Forwarded-For/Proto/Host` hop, only when vouched for), then deletes every `X-Frameleaf-*` request header except `x-frameleaf-worker-session`, the render-worker credential this server issues (FL-95), which is not an arrival claim. A wrong, missing or partial secret, or none configured, leaves the request unmarked, exactly like one that never went through the edge worker; ordinary `X-Forwarded-*` handling (Express `trust proxy`, `IMMICH_TRUSTED_PROXIES`) is unchanged. `main.ts`: the supervisor sets `FRAMELEAF_EDGE_SECRET` to 32 random bytes (base64url) on every bootstrap, including a restart, unless the environment set one, and the workers inherit it.
  - `authenticate()` takes `metadata.via` (from `AuthGuard` and the websocket handshake). For `relay` or `wan`: a shared link passes; a session passes when `immich_fork.frameleaf_session` tags it, or when `frameleafCloud.remoteAccess.allowPasswordOverRelay` is on (the sessions a remote password sign-in creates must then work); an API key passes only when `frameleaf_account_link` has its owner; anything else is 403 `{code: 'frameleaf_sign_in_required'}`. `login()` refuses a password over `relay`/`wan` with the same code, before any account lookup, unless `allowPasswordOverRelay`. Home and unmarked requests never reach either check. `/public/config` and `/server/config` report `signInRequired` false when passwords are allowed there.
  - `@OriginalTransfer()` marks `GET assets/:id/original`, `POST download/archive`, `GET admin/database-backups/:filename` and `GET preservation/packages/:id/download`; `AuthGuard` calls `AuthService.requireOriginalTransfer(via)`, which refuses `via === 'relay'` with 403 `{code: 'frameleaf_relay_originals_refused'}` unless `allowOriginalsOverRelay`. Thumbnails, previews, playback, edit versions and exports are not marked; `wan` is not affected.
  - `middleware/rate-limit.guard.ts` (`APP_GUARD` before `AuthGuard`) with `repositories/rate-limit.repository.ts` (ioredis `MULTI INCR`+`TTL`, then `EXPIRE` when the key has no expiry, so a counter never outlives its window): `@RateLimited(RATE_LIMITS.x)` per client address (IPv6 by /64; the edge worker's forwarded address when vouched for) and per principal (email for `auth/login`, share key or slug for `shared-links/login`, the presented session token or API key otherwise; hashed with SHA-256 before it reaches Redis). Limits, in the admin reference: login 30/address and 10/email per 10 min; OAuth callbacks and link 30 per 10 min; `oauth/frameleaf/*` 60 per 10 min; shared-link login 30/address and 60/link per 10 min; licence activation and certificate install (admin and `users/me/license`) 10 per hour; link start 10 per hour; every `relay`/`wan` request 1,200 per address per minute. Over a limit: 429 `{code: 'rate_limited', retryAfter}` with `Retry-After`. Redis unavailable: remote requests 503, home requests allowed with a warning. `IMMICH_ENV=testing` skips the route limits (the e2e suites sign in hundreds of times from one address) but not the remote ceiling. Counting is per request, not per failure.
  - Shared-link passwords: Frameleaf public migration `2100000000660-HashSharedLinkPasswords` (630 was taken by `AddWorkflowRunStep`; registered in `ORDER` and the migration manifest; no schema change) replaces each plaintext password with its bcrypt hash (cost 10) and an empty one with NULL; `down` keeps the hashes. The service compares bcrypt hashes with `compareBcrypt`, a leftover plaintext value in constant time and then hashes it, derives the unlock cookie token from the stored hash (so the first unlock after the upgrade asks again), compares tokens with `timingSafeEqual`, and never returns the password: `SharedLinkResponseDto.password` is the mask `********` when set, and an edit that sends the mask back keeps the password. Protocol change for the native apps: the password is no longer readable from the API.
  - Websocket: `cors: true` removed; `AuthService.authenticateWebsocket()` refuses a handshake whose `Origin` is not the request's own host (`Host` or `X-Forwarded-Host`), `server.externalDomain`, or a linked server's `services.relayOrigin`/`publicUrl` (skipped in development), reads the arrival from the handshake headers and authenticates with it; `client.data.frameleafVia` carries it. A handshake without `Origin` (apps, CLI) is left to authentication.
  - `frameleafCloud.remoteAccess {allowOriginalsOverRelay, allowPasswordOverRelay}` (both false; reset on unlink or revoke), edited through `PUT admin/cloud/remote-access` (`CloudRemoteAccessUpdateDto`, `adminCloud.update`; turning either on needs a linked server) and shown in `CloudStatusResponseDto`. The web has the prototype's **Who can connect** card as the first part of Settings → Frameleaf Cloud → Remote access (`cloud-remote`, `RemoteAccessSection.svelte`), with its confirmation dialogs; the rest of that page comes with CLD-102.
  - `ServerConfigDto.frameleaf {via, signInAvailable, signInRequired, publicUrl}` and `/.well-known/immich` `frameleaf {instanceId, publicUrl, signIn}` (instance id and address only while linked). `publicUrl` is the link's https `services.publicUrl`, else its `services.relayOrigin`, until CLD-102 publishes the remote-access state.
  - Review fixes (FL-161): a request that claims an arrival (`X-Frameleaf-Via` or `-Via-Auth`) without the right secret is now refused with 403 `frameleaf_via_unverified` (middleware and websocket handshake), never treated as home; a split deployment must set `FRAMELEAF_EDGE_SECRET` explicitly and share it. The per-email and per-link principal limits count 401 failures (`principalCounts: 'failures'`; see the second review fixes), principals are keyed with `CryptoRepository.serverKeyedHash` (HMAC-SHA256 under `<identity dir>/server-hmac.key`, 32 bytes, 0600, never in the database), thumbnails and person thumbnails get their own higher remote ceiling (`@RemoteMediaCeiling()`), the Redis-down 503 carries `Retry-After: 30`, the Redis client connects at start with `enableOfflineQueue: false`, and the guard warns at start under `IMMICH_ENV=testing`. The shared-link unlock token is that HMAC of the link and its stored hash, and a first-use rehash is conditional (`SharedLinkRepository.replaceLegacyPassword`, `WHERE password = <legacy>`). Over the relay a full-size view falls back to the preview instead of redirecting to the original; `@OriginalTransfer()` also covers video version masters, Studio bundles and exports, memory export archives and integrity report files; the render-worker controller is `@HomeNetworkOnly()` (refused over relay and WAN, public routes included). `onConfigValidate` refuses loosening `remoteAccess.*` through the generic settings save on an unlinked server. The web signs out on 403 `frameleaf_sign_in_required` from any call (`handle-error.ts`).
  - Second review fixes (FL-161): failure-counted principals (login email, shared link) are keyed by principal and client address and counted atomically up front (`INCR`, refused over the limit), then given back (`RateLimitRepository.release`, `DECR`) unless the attempt failed with 401, so parallel attempts cannot pass together and nobody elsewhere can lock an address out; `peek` is gone. Thumbnails and person thumbnails count against `REMOTE_MEDIA_CEILING` (6,000 per address per minute, `@RemoteMediaCeiling()`) instead of none. Over the relay a chosen restoration's full-size view serves its preview (`getPlaybackChoice(…, via)`); `assets/:id/develop/revisions/:revisionId/file` and `assets/:id/restorations/:restorationId/file` are `@OriginalTransfer()`. The handoff count runs inside the checkpoint transaction and separates hashed from plaintext passwords (only hashed ones need the acknowledgement). A damaged `server-hmac.key` is replaced by an atomic rename (a new key is linked into place), with a log line that link unlocks reset. `POST auth/logout` is `@RemoteSignInExempt()`.
  - Handoff (P1): migration 660 changes official data the official server reads as plaintext, so after a handoff password-protected links stay locked there (fail closed). `ADOPTION_STEP_COUNTERS` records `passwordProtectedLinks` and `plaintextPasswordLinks` for it; `fork-handoff prepare-official` counts the password-protected links and refuses until `--acknowledge-shared-link-passwords`; the operator resets those passwords in the official app (documented in `upstream-handoff.md` and `server/src/fork-schema/README.md`).
...
`2100000000660-HashSharedLinkPasswords` (630 was taken; FL-161); medium specs per fork table (`recipient-group.repository.spec.ts` pattern).
...
- `websocket.repository.ts`: `cors` → origin allow-list (same-origin, `server.externalDomain`, published names); `on_frameleaf_cloud {topic}` to admins; handshake `frameleafVia`. `shared-link.service.ts`: bcrypt passwords, `timingSafeEqual` tokens (+ migration 660, FL-161). `user.repository.ts` `getAdmins()`; `notification.service.ts` `notifyAdmins({…, dedupeKey, dedupeDays≤30})` + `findRecentByDedupeKey`; the previous GPU-provider integration's service deleted (its `notifyAdmin` callers move to `notifyAdmins`).
...
6. ★ **Remote-access security prerequisites** — rate limits, shared-link password hashing (+660), websocket origin check, via-middleware + supervisor secret, `authenticate()` rule, originals-over-relay refusal. AC: client `X-Frameleaf-Via` dropped; rule enforced; old plaintext links keep working; login throttled.
...
- ★ **CLD-101 Remote-access security prerequisites**. Anchors: `server/src/middleware/rate-limit.guard.ts` (new),
  `server/src/middleware/frameleaf-via.middleware.ts` (new), `server/src/schema/migrations/2100000000660-HashSharedLinkPasswords.ts` (new; 630 was taken),
