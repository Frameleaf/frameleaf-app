...
This workstream completes the Frameleaf phone and tablet applications and qualifies the whole product for release. It preserves existing Immich client connectivity, protocol and database identities while shipping separately identified Frameleaf apps. It is an implementation plan, not a statement that the redesign or release is complete.
...
| Migration and preservation     | Upgrade/rollback, coinstalled backup, existing Immich clients, missing private tables on official, handoff/return, pending leases/jobs, shared ownership, physical-dedup references, checksum-verified restore                                     | QA-104; actual disposable PostgreSQL/official-container lanes |
