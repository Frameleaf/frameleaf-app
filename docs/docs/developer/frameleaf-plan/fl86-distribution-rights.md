...
The standalone attribution audit binds the generated engine lockfile byte-for-byte to the committed `studio/engine-package-lock.json` before deriving its package inventory. A negative regression removes a missing-notice package from both the generated lockfile and artifact index while leaving it installed; the audit rejects this matching tamper. The regression failed before the binding and passes with it.
