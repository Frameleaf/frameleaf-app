...
Frameleaf builds and pulls only its own images. The server and development Dockerfiles build their base (Node, the PostgreSQL clients, jellyfin-ffmpeg, and libvips, ImageMagick, libheif, libjxl, jpegli and LibRaw compiled from pinned revisions) inside the Dockerfile, between the `frameleaf-server-base` markers, from the scripts, version pins and patches in `server/base-image`. They reproduce the former upstream base images of the same revision, and no separate base image is published. `docker/postgres` builds the database image with PostgreSQL 14, VectorChord 0.4.3, pgvector 0.8.1 and pgvecto.rs 0.2.0, the exact versions of the image it replaces, verifying each extension package's checksum. The **Postgres Image** workflow builds and tests it natively on both architectures for pull requests. Development, local production, end-to-end, medium, SQL-schema and certification runs build the database from `docker/postgres` rather than pulling it.

The database and CLI images are published by manual dispatch, not by the release: releases are created with the workflow token, which never triggers other workflows. Before an authorized promotion, dispatch **Postgres Image** (requires `FRAMELEAF_ENABLE_POSTGRES_PUBLISH=true`) and **CLI Build** (requires `FRAMELEAF_ENABLE_CLI_PUBLISH=true`) with publish selected on the exact current `fork/main`, then confirm each package is public. The Postgres publisher pushes the archives its build jobs tested, without rebuilding, records the digest as a job output and a `frameleaf-postgres-digest` artifact, attests an SBOM and build provenance, and refuses to move an existing version tag; a changed image needs a new version tag. The CLI publisher pushes `sha-<commit>` and `latest`. Release promotion checks anonymously, as an installation pulls, that every image the bundled Compose files and the CLI instructions name is published and public, stops with a message naming any that are missing or private, and pins the bundled database reference to the verified digest. The source Compose files keep the version tag, so after the first publication add the recorded digest to them in a normal change.
...
CLI builds are read-only by default. Publication additionally requires the repository variable `FRAMELEAF_ENABLE_CLI_PUBLISH=true`. [Release events created with `GITHUB_TOKEN` do not trigger another release workflow](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows), so the CLI workflow has no release trigger. Publication is the **CLI Build** manual dispatch: select `fork/main` and request publication; it verifies the exact current commit and publishes its SHA tag and `latest`. It does not turn a manual build into a stable release. This variable is not enabled during setup.
