...
:::tip
Development builds its own server base, including the media libraries, from `server/Dockerfile.dev` and the sources in `server/base-image` for its docker-compose based development. For this reason, you won't be able to use VSCode's **_Clone Repository in a Container Volume_** command.
:::
