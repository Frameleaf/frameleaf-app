...
import MobileArchitecture from './img/mobile-architecture.svg';
...
graph LR
    subgraph Clients
        ...
        C["Frameleaf CLI (TypeScript)"]
    end
    subgraph Server
        API[API controllers]
        CORE["Core services (domain)"]
    ...
    JOBS --> CORE
    CORE --> REPO
    REPO -- HTTP --> ML["Machine learning (Python)"]
    REPO --> FS[File system]
    REPO --> PG[(Postgres)]
    REPO --> RD[(Redis)]
    RD -- Queue --> JOBS
...
Frameleaf has three main clients:
...
Frameleaf uses [Redis](https://redis.com/) via [BullMQ](https://docs.bullmq.io/) to manage job queues. Some jobs trigger subsequent jobs. For example, Smart Search and Facial Recognition relies on thumbnail generation and automatically run after one is generated.
