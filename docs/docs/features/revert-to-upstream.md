...
- a compatible fork release before returning from the official upstream server.
...
Users of the upstream server upgrading directly to this fork retain their official data. Fork sidecars are initialized from public rows or safe defaults; official workflow data is left unchanged.
