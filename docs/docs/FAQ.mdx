...
Frameleaf is available as open source under the terms of the GNU AGPL v3 License, like the upstream project it is built on. See the [acknowledgements](/overview/acknowledgements) for details.
...
### How can I see a list of all users in Frameleaf?
...
On iOS (iPhone and iPad), the operating system determines if a particular app can invoke background tasks based on multiple factors, most of which the mobile app has no control over. To increase the likelihood that the background backup task is run, follow the steps below:

- Enable Background App Refresh for the mobile app in the iOS settings at `Settings > General > Background App Refresh`.
- Disable **Low Power Mode** when not needed, as this can prevent apps from running in the background.
- Disable Background App Refresh for apps that don't need background tasks to run. This will reduce the competition for background task invocation for the mobile app.
- Use the mobile app more often.
...
Many of these limitations are known issues in the upstream project.
...
### Does Frameleaf change the file?

No, Frameleaf does not modify the original files.
All edited metadata is saved in companion `.xmp` sidecar files and the database.
However, Frameleaf will delete original files that have been trashed when the trash is emptied in the Frameleaf UI.

### Why do my file names appear as a random string in the file manager?

When Storage Template is off (default) Frameleaf saves the file names in a random string (also known as random UUIDs) to prevent duplicate file names.
...
### Why are only photos and not videos being uploaded to Frameleaf?

This often happens when using a reverse proxy in front of Frameleaf.
...
### How can I backup data from Frameleaf?

See [Backup and Restore](/administration/backup-and-restore.md).

### Does Frameleaf support reading existing face tag metadata?

Yes, it creates new faces and persons from the imported asset metadata.

### Does Frameleaf support the filtering of NSFW images?

Frameleaf can optionally detect NSFW images and hide privately flagged NSFW assets from library views until the current session is unlocked with the locked-folder PIN. Hiding uses the private NSFW flag, not the visible `nsfw` tag, and album membership is preserved.
...
Files sent on WhatsApp are saved without metadata on the file. Therefore, Frameleaf has no way of knowing the original date of the file when files are uploaded from WhatsApp,
not the order of arrival on the device.
...
### Why do HDR videos appear pale in the Frameleaf player but look normal after download?

Frameleaf uses a player with known HDR color display issues. We are experimenting with a different player that provides better color profiles for HDR content for future improvements.

### Why does Frameleaf transcode my videos to a lower quality?

Frameleaf always keeps your original files. Alongside that, it generates a transcoded version for compatibility and performance reasons.
...
### Can I keep my existing album structure while importing assets into Frameleaf?

Yes, by using the [command-line interface](/features/command-line-interface) along with the `--album` flag.
...
In read-only libraries (`:ro` in the `docker-compose.yml`), Frameleaf is unable to create the `.xmp` sidecar files to store edited file metadata.
...
Frameleaf will attempt to delete original files that have been trashed when the trash is emptied.
In read-write external libraries (the default), Frameleaf will delete the original file.
In read-only libraries (`:ro` in the `docker-compose.yml`), files can still be trashed in the UI.
However, when the trash is emptied, the files will re-appear in the main timeline since Frameleaf is unable to delete the original file.
...
Frameleaf uses CLIP models. An ML model converts each image to an "embedding", which is essentially a string of numbers that semantically encodes what is in the image. The same is done for the text that you enter when you do a search, and that text embedding is then compared with those of the images to find similar ones. Smart Search itself does not generate visible tags, labels, or descriptions. If image enrichment is enabled, Frameleaf can run separate ML jobs to generate searchable descriptions and tags. For more information about CLIP and its capabilities, read about it [here](https://openai.com/research/clip).
...
You can delete the model cache volume, where models are downloaded. This will give the service a clean environment to download the model again. If models are failing to download entirely, you can manually download them from the model source (by default the Frameleaf model mirror at [models.frameleaf.cloud][model-source], or the host set in `MACHINE_LEARNING_MODEL_SOURCE_URL`) and place them in the cache folder.

### Can I use a custom CLIP model?

No, this is not supported. Only models listed in [Searching](/features/searching#clip-models) are compatible. Feel free to make a feature request if there's a model not listed here that you think should be added.

### I want to be able to search in other languages besides English. How can I do that?

You can change to a multilingual CLIP model. See [here](/features/searching#clip-models) for instructions.

### Does Frameleaf support Facial Recognition for videos?

Frameleaf's machine learning feature operates on the generated thumbnail. If a face is visible in the video's thumbnail it will be picked up by facial recognition.
Scanning the entire video for faces may be implemented in the future.

### Does Frameleaf have animal recognition?
...
### Why is Frameleaf slow on low-memory systems like the Raspberry Pi?

Frameleaf optionally uses transcoding and machine learning for several features. However, it can be too heavy to run on a Raspberry Pi. You can [mitigate](/FAQ#can-i-lower-cpu-and-ram-usage) this or host Frameleaf's machine-learning container on a [more powerful system](/guides/remote-machine-learning), or [disable](/FAQ#how-can-i-disable-machine-learning) machine learning entirely.
...
### How can I see Frameleaf logs?

Frameleaf components are typically deployed using docker. To see logs for deployed docker containers, you can use the [Docker CLI](https://docs.docker.com/engine/reference/commandline/cli/), specifically the `docker logs` command. For examples, see [Docker Help](/guides/docker-help.md).
...
### How can I run Frameleaf as a non-root user?

You can change the user in the container by setting the `user` argument in `docker-compose.yml` for each service.

[Example docker-compose.rootless.yml file](https://github.com/Frameleaf/frameleaf-app/releases/latest/download/docker-compose.rootless.yml)
...
### How can I purge data from Frameleaf?

Data for Frameleaf comes in two forms:
...
```bash title="Remove Frameleaf (containers and volumes)"
docker compose down -v
```

After removing the containers and volumes, there are a few directories that need to be deleted to reset Frameleaf to a new installation. Once they are deleted, Frameleaf can be started back up and will be a fresh installation.

- `DB_DATA_LOCATION` contains the database, media info, and settings.
- `UPLOAD_LOCATION` contains all the media uploaded to Frameleaf.

:::note Portainer
If you use portainer, bring down the stack in portainer. Go into the volumes section  
and remove all the volumes related to Frameleaf, then restart the stack.
:::
...
If it mentions SIGILL (note the lack of a K) or error code 132, it most likely means your server's CPU is incompatible with Frameleaf.
...
### Will Frameleaf save me money?

Probably not. At first glance, it seems like it might: compared to cloud-hosted commercial services, there are no monthly fees to run Frameleaf on your own server, and a supporter key is optional. However, there are many other costs like hardware, storage for backups, and your own time. There are many good reasons for self-hosting, but saving money is rarely one of them.

### I really want a particular feature. Can I sponsor development of it?

No, we have no feature sponsorship scheme. Frameleaf is funded by the people who use it: [supporting Frameleaf](/overview/support-the-project#supporter-key) with a supporter key gives access to Frameleaf Cloud, enhanced machine learning features and more. We'd rather not let money influence the roadmap. If features could be bought, we'd end up building Frameleaf for the highest bidder instead of for all of our users.
...
[model-source]: https://models.frameleaf.cloud
