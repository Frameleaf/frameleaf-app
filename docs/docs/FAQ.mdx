...
Frameleaf is built on Immich and, like it, is available as open source under the terms of the GNU AGPL v3 License.
...
This fork can optionally detect NSFW images and hide privately flagged NSFW assets from library views until the current session is unlocked with the locked-folder PIN. Hiding uses the private NSFW flag, not the visible `nsfw` tag, and album membership is preserved. There is an [open upstream feature request on Github](https://github.com/immich-app/immich/discussions/2451).
