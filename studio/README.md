...
This directory contains preservation metadata and an explicit isolated engine build. The source archive is recovered only by the preparation command below; the vendor snapshot and generated workspace are not committed. There is no application-integrated editor, deployed rendering worker, hardware qualification or licensed Dolby tools. Locked npm packages may contain payloads recorded in the FL-86 resource inventory; no separate weight acquisition is authorized by this build.
