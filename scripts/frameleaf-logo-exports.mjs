#!/usr/bin/env node
// Renders Frameleaf logo exports for web, docs and mobile from the supplied brand kit (never modified)
// and records each output's provenance in design/frameleaf/derivatives/exports.json.
// Usage: node scripts/frameleaf-logo-exports.mjs
...
// Web
for (const name of ["frameleaf-symbol.svg", "frameleaf-logo-dark.svg"]) {
  copy(`${kit}/${name}`, `web/static/frameleaf/${name}`);
}
copy(src.logoLight, "web/static/frameleaf/frameleaf-logo-light.svg");
await png(
...
await png("mobile/assets/frameleaf-wordmark-light.png", src.logoLight, 0, 400, {
  trimLeft: 280 / 1080,
});
