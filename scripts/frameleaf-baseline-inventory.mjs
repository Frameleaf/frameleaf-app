...
function hashGitDirectory(repository, absolutePath, entry) {
  ...
  const headCommit =
    tryGit(absolutePath, ["rev-parse", "HEAD"])?.trim() ?? null;
  if (!indexCommit && !headCommit) {
    throw new Error(
      `Status directory is not a Git repository or gitlink: ${entry.filePath}`,
    );
  }
  ...
  for (const nestedPath of untrackedPaths
    .toString("utf8")
    .split("\0")
    .filter(Boolean)) {
    ...
    if (nestedStat.isDirectory()) {
      throw new Error(
        `Nested untracked directory was not expanded: ${entry.filePath}/${nestedPath}`,
      );
    }
...
export function buildReport({ acceptedBaseline, sourceRepository }) {
  ...
  const parsed = parsePorcelain(firstStatus)
    .map((entry) => ({
      ...entry,
      ...classifyPath(entry.filePath, entry.indexStatus, entry.worktreeStatus),
    }))
    .sort((left, right) =>
  ...
  if (!secondStatus.equals(thirdStatus)) {
    throw new Error(
      "Working-tree status changed during inventory verification",
    );
  }
  ...
  if (JSON.stringify(firstIdentity) !== JSON.stringify(secondIdentity)) {
    throw new Error(
      "Repository identity or remotes changed during inventory generation",
    );
  }
...
export async function formatGeneratedContent(filePath, content) {
  const prettier = await import("prettier");
  const sortJsonPlugin = await import("prettier-plugin-sort-json");
  const config = (await prettier.resolveConfig(filePath)) ?? {};
  return prettier.format(content, {
    ...config,
    filepath: filePath,
    plugins: [sortJsonPlugin.default ?? sortJsonPlugin],
  });
}
...
async function main() {
  ...
  writeFileSync(options["output-json"], json, "utf8");
  writeFileSync(options["output-markdown"], markdown, "utf8");
