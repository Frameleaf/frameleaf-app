...
class Matrix {
  ...
  drawFunctionPatterns(version, ecc) {
  ...
  finderPenaltyTerminate(runColor, runLength, history) {
    if (runColor) {
  ...
  penaltyScore() {
    ...
    for (let y = 0; y < size; y++) {
      ...
      result +=
        this.finderPenaltyTerminate(runColor, runX, history) * PENALTY_N3;
    }
    for (let x = 0; x < size; x++) {
      let runColor = false;
      let runY = 0;
      ...
      result +=
        this.finderPenaltyTerminate(runColor, runY, history) * PENALTY_N3;
    }
...
export function selectVersion(byteLength, ecc, minVersion = MIN_VERSION) {
  assertEcc(ecc);
 ...
 * @returns {{ version:number, size:number, ecc:"L"|"M", mask:number, modules:boolean[][] }}
 */
export function encodeQr(text, { ecc = "M", minVersion = MIN_VERSION } = {}) {
  ...
  const codewords = addEccAndInterleave(
    buildCodewords(bytes, version, ecc),
    version,
    ecc,
  );
